function stats = extract_stats_segmentation_Mod2(net, stats)
    ...
    % added
    sel2 = find(cellfun(@(x) isa(x,'dagnn.LossDice'), {net.layers.block})) ;
    obj2 = net.vars(net.layers(sel2).outputIndexes(1)).value;
    ...
    if(isfield(stats, 'objective'))
        stats.objective = obj + obj2 + stats.objective;
    else
        stats.objective = obj + obj2;
    end
